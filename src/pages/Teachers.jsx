  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          <h2 className="text-xl md:text-2xl font-bold text-slate-900">{t('teachers.heading')}</h2>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          {user.branch === 'all' && (
            <select value={branchFilter} onChange={(e) => setBranchFilter(e.target.value)}
              ...
              <option value="all">{t('teachers.filter_all_branches')}</option>
          ...
          {canAdd && (
            <button onClick={() => { setEditingTeacher(null); setModalOpen(true) }}
              ...
              <Plus size={16} /> {t('teachers.btn_add')}
            </button>
          )}
        </div>
      </div>
      ...
      {canSeeSalaries && (
        <div className="glass-card rounded-2xl p-6">
          <h3 className="text-lg font-semibold text-slate-900 mb-4">{t('teachers.summary_title')}</h3>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-white/40 border-b border-white/30">
                  <th className="text-left py-3 px-4 text-slate-500 font-medium">{t('teachers.th_branch')}</th>
                  <th className="text-center py-3 px-4 text-slate-500 font-medium">{t('teachers.th_teachers')}</th>
                  <th className="text-center py-3 px-4 text-slate-500 font-medium hidden md:table-cell">{t('teachers.th_total_groups')}</th>
                  <th className="text-center py-3 px-4 text-slate-500 font-medium hidden md:table-cell">{t('teachers.th_total_students')}</th>
                  <th className="text-right py-3 px-4 text-slate-500 font-medium">{t('teachers.th_fot')}</th>
                  <th className="text-center py-3 px-4 text-slate-500 font-medium hidden md:table-cell">{t('teachers.th_avg_rating')}</th>
                </tr>
              </thead>
              <tbody>
                {branches.map((branch) => {
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editingTeacher ? t('teachers.modal_edit') : t('teachers.modal_new')} size="lg">
        <TeacherForm teacher={editingTeacher} onClose={() => setModalOpen(false)} />
      </Modal>

      <Modal isOpen={!!confirmDelete} onClose={() => setConfirmDelete(null)} title={t('teachers.modal_confirm_delete')} size="sm">
        <p className="text-sm text-slate-600 mb-4">{t('teachers.confirm_delete')}</p>
        <div className="flex justify-end gap-3">
          <button onClick={() => setConfirmDelete(null)} className="px-4 py-2 text-sm font-medium text-slate-600 bg-slate-100 rounded-lg hover:bg-slate-200">{t('teachers.btn_cancel')}</button>
          <button onClick={() => { deleteTeacher(confirmDelete); setConfirmDelete(null) }} className="px-4 py-2 text-sm font-medium text-white bg-red-600 rounded-lg hover:bg-red-700">{t('teachers.btn_delete')}</button>
        </div>
      </Modal>
    </div>
  )
