...
// ─── helpers ──────────────────────────────────────────────────────────────────
const STATUS = {
  done: { label: 'Выполнен',   bg: 'bg-emerald-100', text: 'text-emerald-700', bar: 'bg-emerald-500' },
  good: { label: 'По плану',   bg: 'bg-blue-100',    text: 'text-blue-700',    bar: 'bg-blue-500'    },
  risk: { label: 'Под риском',  bg: 'bg-amber-100',   text: 'text-amber-700',   bar: 'bg-amber-500'   },
  low:  { label: 'Отстаёт',    bg: 'bg-red-100',     text: 'text-red-700',     bar: 'bg-red-400'     },
}
  ...
  return (
    <div className={`bg-white rounded-xl p-5 shadow-sm border border-slate-100 ring-1 ${rings[accent] || rings.blue}`}>
      ...
      <div className="flex items-end justify-between mb-3">
        <div>
          <p className="text-xs text-slate-400 mb-0.5">Достигнуто</p>
          <p className="text-xl font-bold text-slate-900">{formatCurrency(perf.achieved)}</p>
        </div>
        <div className="text-right">
          <p className="text-xs text-slate-400 mb-0.5">План</p>
  ...
  return (
    <div className={`bg-white rounded-xl p-4 shadow-sm border transition-shadow hover:shadow-md ${isMe ? 'border-blue-300 ring-1 ring-blue-200' : 'border-slate-100'}`}>
      <div className="flex items-center gap-3 mb-3">
        ...
        <div className="min-w-0 flex-1">
          <p className="text-sm font-semibold text-slate-900 truncate">
            {manager.name}
            {isMe && <span className="ml-1 text-xs text-blue-500 font-normal">(вы)</span>}
          </p>
          {perf.plan === 0 && <p className="text-xs text-slate-400">Нет плана</p>}
        </div>
        <StatusBadge status={perf.status} />
      </div>
      <ProgressBar pct={perf.percentage} status={perf.status} />
      <div className="grid grid-cols-3 gap-2 mt-3 text-center">
        <div><p className="text-xs text-slate-400">План</p><p className="text-xs font-semibold text-slate-700">{perf.plan > 0 ? formatCurrency(perf.plan) : '—'}</p></div>
        <div><p className="text-xs text-slate-400">Факт</p><p className="text-xs font-semibold text-emerald-600">{perf.achieved > 0 ? formatCurrency(perf.achieved) : '—'}</p></div>
      ...
      {perf.plan > 0 && perf.remaining > 0 && (
        <p className="mt-2 text-xs text-slate-400 text-center">Осталось: <span className="font-medium text-slate-600">{formatCurrency(perf.remaining)}</span></p>
      )}
    </div>
  )
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        {isAdmin && (
          <div className="flex items-center gap-2">
            <span className="text-sm font-medium text-slate-600">Филиал:</span>
        ...
        <div className="ml-auto">
          {isAdmin && (
            <button onClick={() => setPlanModalOpen(true)}
              ...
              <Settings size={15} /> Установить план
            </button>
          )}
        </div>
      </div>
      ...
      {noPlan && (
        <div className="flex items-start gap-3 bg-amber-50 border border-amber-200 rounded-xl p-4">
          ...
          <div>
            <p className="text-sm font-semibold text-amber-800">Плановые показатели не установлены</p>
            <p className="text-xs text-amber-700 mt-0.5">
              {isAdmin ? 'Нажмите «Установить план» чтобы задать цели.' : 'Обратитесь к администратору.'}
            </p>
          </div>
        </div>
      )}
      ...
      {isAdmin ? (
        <div className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-${Math.min(branches.length + 1, 4)} gap-4`}>
          <SummaryCard title="Весь отдел продаж" perf={teamPerf} accent="blue" />
          {branches.map((b, i) => {
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <SummaryCard title={`Филиал: ${getBranchName(user.branch)}`} perf={branchPerfs[user.branch] || teamPerf} accent="blue" />
          {user.managerId && (() => {
            ...
            return (
              <div className="bg-gradient-to-br from-blue-600 to-blue-700 rounded-xl p-5 text-white shadow-sm">
                <div className="flex items-center gap-2 mb-3"><Award size={18} className="opacity-80" /><p className="text-sm font-medium opacity-90">Мои показатели</p></div>
                <div className="flex items-end justify-between mb-3">
                  <div><p className="text-xs opacity-70 mb-0.5">Достигнуто</p><p className="text-2xl font-bold">{formatCurrency(perf.achieved)}</p></div>
                  <div className="text-right"><p className="text-xs opacity-70 mb-0.5">План</p><p className="text-base font-semibold opacity-90">{perf.plan > 0 ? formatCurrency(perf.plan) : '—'}</p></div>
                </div>
                ...
                <div className="flex items-center justify-between">
                  <span className="text-xs opacity-80">{perf.remaining > 0 ? `Осталось ${formatCurrency(perf.remaining)}` : 'План выполнен!'}</span>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-base font-semibold text-slate-900 flex items-center gap-2">
            ...
            {isAdmin ? 'Показатели менеджеров' : 'Команда'}
          </h3>
          <span className="text-xs text-slate-400">{visibleManagers.length} сотрудников</span>
        </div>
